...
bucket_name="Ajay-ultimate-bucket"
lambda_func_name="s3-lambda-function"
role_name="s3-lambda-sns"
email_address="xyz@gmail.com"

# Create IAM Role for the project
role_response=$(aws iam create-role --role-name s3-lambda-sns --assume-role-policy-document '{   // IAM role named s3-lambda-sns and stores the response in the role_response variable. The assume-role-policy-document specifies that the role can be assumed by the Lambda, S3, and SNS services.
  "Version": "2012-10-17",
  "Statement": [{
    ...
    "Principal": {
      "Service": [
         "lambda.amazonaws.com",    // why the whole …com - The full service endpoint is used to explicitly specify the service. 
         "s3.amazonaws.com",
         "sns.amazonaws.com"
      ]z
    }
...
# Create a Zip file to upload Lambda Function - Since Lambda function has multiple files we Zip
...
aws s3api put-bucket-notification-configuration \
  ...
  --notification-configuration '{
    "LambdaFunctionConfigurations": [{
        ...
        "Events": ["s3:ObjectCreated:*"]                                              ///When object is created  Lambda triggers
    }]
}'

# Create an SNS topic and extracts the topic ARN to a variable
